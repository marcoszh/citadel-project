...
def generate_gradient(worker_id):
    ...
    is_last = request.json['is_last']
    iter_num = int(request.json['iter_num'])
    model_file = download_decrypt_blob('model', 'alexnet_{0:03d}'.format(iter_num), 'alexnet_{0:03d}'.format(iter_num),
                                       encryptor_m)
    ...
    # request mask
    response = requests.post(MASK_URL + str(worker_id), json={'shapes': json.dumps(shapes), 'is_last': is_last})
